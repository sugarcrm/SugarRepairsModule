    ...
    public function getInstalledModules()
    {
        ...
        while ($module = $GLOBALS['db']->fetchByAssoc($result)) {
            try {
                ...
                echo "bad serialization on module: {$module['name']}<br>This is going to throw off the accuracy of this tool<br>";
            }
            if (isset($manifest['installdefs']['copy'])) {
                foreach ($manifest['installdefs']['copy'] as $set) {
                    ...
                    if (!array_key_exists($md5Name, $this->MD5) && file_exists($fileName)) {
