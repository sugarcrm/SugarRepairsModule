...
class FileScanner
{
    ...
    public $MD5;
    public $issues = array();
    private $installedModulesFiles = array();
    private $validExt = array('log', 'htaccess', 'manifest', 'ttf', 'ico', 'svg', 'json');
    private $ignoreDirectories = array('cache', 'upload');
    private $sugarModuleList = array("MigrationScanner", "HealthCheck", "SugarHeartbeat", "SugarSystemInfo");
    ...
    public function __construct($filePath, $scannerPath = '')
    {
        if (!empty($filePath)) {
            ...
            die("Path to SugarCRM files required");
        }
        if (!empty($scannerPath)) {
            if (is_file($scannerPath)) {
                $this->scannerPath = $scannerPath;
            } else {
                die("Valid path to ModuleInstall/ModuleScanner.php required");
            }
            ...
            die("Path to ModuleInstall/ModuleScanner.php required");
        }
    ...
    private function setMD5($path)
    {
        ...
        foreach ($md5_string as $path => $md5) {
            $this->MD5[$path] = $md5;
        }
    }
